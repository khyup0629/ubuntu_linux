...
* EXT4 저널링 파일 시스템   
파일 시스템이 정상적으로 종료(언마운트)되지 않았다면, 저장장치 내에 있는 모든 INode와 Bitmap 등의 Meta Data를 검사하여 일관성 문제를 해결합니다.   
그러나 모두 검사하기에는 시간이 너무 오래 걸리는 이슈가 있습니다.
이런 상황을 해결하고자 파일시스템 Ext3부터 `저널링 방식`을 적용하였습니다.   
`파일을 쓰는 작업`을 할 때 파일시스템의 특정 영역인 `저널(Journal)`이라 불리는 로그를 기록한 뒤,   
작업이 정상적으로 완료 되었을 때 커밋(commit)을 해서 실제 사용되는 영역에 기록합니다.   
이 방법을 사용하면 시스템이 중간에 종료가 되어도 종료된 위치를 바로 알 수 있기 때문에 파일시스템 전체를 검사할 필요가 없게 됩니다.
